...
def run_strategy():
    ...
    learner = StrategyLearner(verbose=False, impact=impact1)
    ...
    learn_val = market.compute_portvals(df_learner_trades, start_val=100000, commission=commision, impact=impact1)
    learn_val_normalized = learn_val[:]/learn_val[0]

    manual_val = market.compute_portvals(df_manual_trades, start_val=100000, commission=commision, impact=impact1)
    manual_val_normalized = manual_val[:]/manual_val[0]

    benchmark = get_benchmark_df('JPM', price.index, ed, 1000)
    benchmark_val = market.compute_portvals(benchmark, start_val=100000, commission=commision, impact=impact1)
