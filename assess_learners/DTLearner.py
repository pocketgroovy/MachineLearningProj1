...
class DTLearner(object):
    ...
    def build_tree(self, dataX, dataY):
        if dataX.shape[0] <= self.leaf_size:
            ...
            correl_array = self.get_correlational_array(dataX, dataY)
            factor_index = self.get_factor_index(abs(correl_array))
    ...
    def get_correlational_array(self, dataX, dataY):
        correl_mat = np.corrcoef(dataX.T, dataY)
        correl_array = correl_mat[-1, :-1]
        return correl_array


    def get_factor_index(self, a):
        return a.argmax()
