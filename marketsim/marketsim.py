...
def compute_portvals(orders_file = "./orders/orders.csv", start_val = 1000000, commission=9.95, impact=0.005):
    ...
    # TODO: Your code here
    df_data = pd.read_csv(orders_file, index_col='Date', parse_dates=True, na_values=['nan']).sort_index()
    ...
    for index, row in df_data_copy.iterrows():
        if row['Order'] == 'BUY':
            df_port_val[index:] = df_port_val[index:] - (((df_prices.ix[index, row['Symbol']] * row['Shares']) * impact) + commission)
        if row['Order'] == 'SELL':
            df_port_val[index:] = df_port_val[index:] + (((df_prices.ix[index, row['Symbol']] * row['Shares']) * impact) - commission)
